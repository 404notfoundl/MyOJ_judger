...
int CRabbitmqClient::Connect(const string& strHostname, int iPort, const string& strUser, const string& strPasswd, int channel) {
	...
	if (0 != ErrorMsg(amqp_login(m_pConn, "/", 0, AMQP_DEFAULT_FRAME_SIZE, HEART_BEAT_TIME, AMQP_SASL_METHOD_PLAIN, m_strUser.c_str(), m_strPasswd.c_str()), "Logging in")) {
		return -4;
	}
...
int CRabbitmqClient::ExchangeDeclare(const string& strExchange, const string& strType, int durable) {
...
int CRabbitmqClient::QueueDeclare(const string& strQueueName, int durable, int auto_delete) {
	if (NULL == m_pConn) {
