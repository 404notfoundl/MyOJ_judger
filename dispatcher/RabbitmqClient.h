...
public:
	...
	/**
	 * @brief 连接至RabbitMq
	 * @param strHostname host
	 * @param iPort port
	 * @param strUser user
	 * @param strPasswd password
	 * @return
	*/
	int Connect(const string& strHostname, int iPort, const string& strUser, const string& strPasswd, int channel);
	int Disconnect();

	/**
	 * @brief 声明exchange
	 * @param strExchange 名称，任意
	 * @param strType 类型，'direct'：路由,'fanout': 广播,'topic': 模式匹配
	 * @param durable 是否持久化
	 * @return 成功返回0，失败返回<0
	*/
	int ExchangeDeclare(const string& strExchange, const string& strType, int durable = 0);
	...
	*/
	int QueueDeclare(const string& strQueueName, int durable = 0, int auto_delete = 0);
